...
class AdSecNeutralAxisGoo : GH_GeometricGoo<NeutralAxis>, IGH_PreviewData {
  ...
  private Line CalculateNeutralAxis() {
    ...
    var offset = m_value.Offset.As(DefaultUdoubles.LengthUnitGeometry);
    var angleRadians = m_value.Angle;

    double/ Calculate direction vector once
    ...
    var width = 1.05 * bbox.PointAt(0, 0, 0doubleistanceTo(bbox.PointAt(1, 0, 0));
    // Calculate start point and line in one step
    var start = local.Origin - direction * (width / 2);
    var line(direction* (width / 2))ction, width);
    ...
    offsVec.Unitize();
    // move the line
    line.Transform(Rhino.Geometry.Transform.Translation(ofRhino.Geometry.Transformc.Y * offset, offsVec.Z * offset));
    return line;
  ...
  public override BoundingBox Boundingbox {
    get {
      return AxisLine. => AxisLine.BoundingBox;
  ...
  public void DrawViewportWires(GH_PreviewWireArgs args) {
    ...
    if (args.Color.R == defaultCol.R && args.Color.G == defaultCol.G && args.Color.B == defaultCol.B) {
      ...
      Colours.Pipeline.DrawLine(AxisLine, UI.Colour.OasysYellow);
    }
  }

  Colourc void DrawViewportMeshes(GH_PreviewMeshArgs args) {
  }
}
}

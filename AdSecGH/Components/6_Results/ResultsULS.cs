...
namespace AdSecGH.Components {
  public class ResultsULS : GH_OasysComponent {
    ...
    public ResultsULS() : base("Strength Result", "ULS",
      "Performs strength checks (ULS), for a given Load or Deformation.", CategoryName.Name(), SubCategoryName.Cat7()) {
    ...
    protected override void RegisterInputParams(GH_InputParamManager pManager) {
      pManager.AddGenericParameter("Results", "Res", "AdSec Results to perform strenght check on.",
        GH_ParamAccess.item);
    ...
    protected override void RegisterOutputParams(GH_OutputParamManager pManager) {
      ...
      pManager.AddGenericParameter("Neutral Axis Offset", "NaO",
        "The Offset of the Neutral Axis from the Sections centroid", GH_ParamAccess.item);
      pManager.AddNumberParameter("Neutral Axis Angle", "NaA",
      ...
      pManager.AddGenericParameter("Failure Neutral Axis Offset", "FaO",
        "The Offset of the Neutral Axis at failure from the Sections centroid", GH_ParamAccess.item);
      pManager.AddNumberParameter("Failure Neutral Axis Angle", "FaA",
        "The Angle [rad] of the Neutral Axis at failure from the Sections centroid", GH_ParamAccess.item);
    }
    ...
    protected override void SolveInstance(IGH_DataAccess DA) {
      ...
      var ulsDeformationResult = uls.Deformation;
      DA.SetData(2,
        new Vector3d(ulsDeformationResult.X.As(DefaultUnits.StrainUnitResult),
      ...
      foreach (var mrng in uls.MomentRanges) {
        var interval = new Interval(mrng.Min.As(DefaultUnits.MomentUnit), mrng.Max.As(DefaultUnits.MomentUnit));
      ...
      var offset = CalculateOffset(ulsDeformationResult);
      ...
      var failureDeformationResult = failure.Deformation;
      DA.SetData(8,
        new Vector3d(failureDeformationResult.X.As(DefaultUnits.StrainUnitResult),
          failureDeformationResult.YY.As(DefaultUnits.CurvatureUnit),
          failureDeformationResult.ZZ.As(DefaultUnits.CurvatureUnit)));
      var offsetFailure = CalculateOffset(failureDeformationResult);

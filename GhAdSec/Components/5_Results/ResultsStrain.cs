...
{
    public class ResultStrain : GH_Component
    {
        ...
        public ResultStrain()
          : base("Strain Result", "ε", "Calculate the strain at a point on the Section for a given Load or Deformation.",
        ...
        protected override void SolveInstance(IGH_DataAccess DA)
        {
            ...
            GH_UnitNumber outStrainULS = new GH_UnitNumber(new Oasys.Units.Strain(strainULS.As(Units.StrainUnit), Units.StrainUnit));
            DA.SetData(0, outStrainULS);

            Pressure stressULS = solution.m_section.Section.Material.Strength.StressAt(strainULS);
            GH_UnitNumber outStressULS = new GH_UnitNumber(new UnitsNet.Pressure(stressULS.As(Units.StressUnit), Units.StressUnit));
            DA.SetData(1, outStressULS);

            Strain strainSLS = sls.Deformation.StrainAt(GetInput.IPoint(this, DA, 2, false));
            GH_UnitNumber outStrainSLS = new GH_UnitNumber(new Oasys.Units.Strain(strainSLS.As(Units.StrainUnit), Units.StrainUnit));
            DA.SetData(2, outStrainSLS);

            Pressure stressSLS = solution.m_section.Section.Material.Serviceability.StressAt(strainSLS);
            GH_UnitNumber outStressSLS = new GH_UnitNumber(new UnitsNet.Pressure(stressSLS.As(Units.StressUnit), Units.StressUnit));

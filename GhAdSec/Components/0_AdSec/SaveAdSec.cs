    ...
    protected override void SolveInstance(IGH_DataAccess DA)
    {
      ...
      if (DA.GetDataTree(1, out GH_Structure<IGH_Goo> loads))
      {
        if (loads.Branches.Count > 0)
        {
          ...
          {
            int loadID = sectionID;
            if (loadID > loads.Branches.Count - 1)
            {
              loadID = loads.Branches.Count - 1; // if list of loads != list of sections, use last item in load (default GH behaviour)
              AddRuntimeMessage(GH_RuntimeMessageLevel.Remark, "Note that the last load in the list has been applied to remaining sections");
            }

            if (loads.Branches[loadID] == null | loads.Branches[loadID].Count == 0)
            {
              // convert to json without loads method
              try
              {
                jsonStrings.Add(json.SectionToJson(sections[loadID].Section));
              }
              catch (Exception e)
              {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "Error with section at index " + loadID + ": " + e.InnerException.Message);
              }
            }
            else if (loads.Branches[loadID][0].CastTo(out AdSecLoadGoo notusedLoad))
            {
              ...
              for (int j = 0; j < loads.Branches[loadID].Count; j++)
              {
                // check if item is load type
                if (loads[loadID][j].CastTo(out AdSecLoadGoo loadGoo))
                {
                ...
                {
                  AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "Unable to convert " + Params.Input[1].NickName + " path " + loadID + " index " + j + " to AdSec Load. Section will be saved without this load.");
                }
              }
              // convert to json with load method
              try
              {
                jsonStrings.Add(json.SectionToJson(sections[sectionID].Section, lds));

              }
              catch (Exception e)
              {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "Error with section at index " + sectionID + ": " + e.InnerException.Message);
              }
            }
            else if (loads.Branches[loadID][0] is AdSecDeformationGoo)
            {
              ...
              {
                // check if item is load type
                if (loads[loadID][j] is AdSecDeformationGoo)
                {
                  AdSecDeformationGoo def = (AdSecDeformationGoo)loads[loadID][j];
                  defs.Add(def.Value);
                }
                else
                {
              ...
              try
              {
                jsonStrings.Add(json.SectionToJson(sections[sectionID].Section, defs));
              }
              catch (Exception e)
              {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "Error with section at index " + sectionID + ": " + e.InnerException.Message);
            ...
            else
            {
              ...
              try
              {
                jsonStrings.Add(json.SectionToJson(sections[sectionID].Section));
              }
              catch (Exception e)
              {
                AddRuntimeMessage(GH_RuntimeMessageLevel.Warning, "Error with section at index " + sectionID + ": " + e.InnerException.Message);
        ...
        else
        {
          for (int i = 0; i < sections.Count; i++)
          {
            ...
            {
              AddRuntimeMessage(GH_RuntimeMessageLevel.Error, "Error with section at index " + i + ": " + e.InnerException.Message);
            }
          }
        }
      }

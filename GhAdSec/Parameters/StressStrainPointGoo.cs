    ...
    public class AdSecStressStrainPointGoo : GH_GeometricGoo<Point3d>, IGH_PreviewData
    {
        ...
        : base(point)
        {
            ...
            this.m_SSpoint = IStressStrainPoint.Create(
                new UnitsNet.Pressure(m_value.Y, AdSecGH.DocumentUnits.StressUnit), 
                new Oasys.Units.Strain(m_value.X, AdSecGH.DocumentUnits.StrainUnit));
        }
        ...
        public AdSecStressStrainPointGoo(IStressStrainPoint stressstrainPoint)
        {
            ...
            this.m_value = new Point3d(
                m_SSpoint.Strain.As(AdSecGH.DocumentUnits.StrainUnit),
                m_SSpoint.Stress.As(AdSecGH.DocumentUnits.StressUnit),
                0);
        }
        ...
        public static IStressStrainPoint CreateFromPoint3d(Point3d point)
        {
            return IStressStrainPoint.Create(
                new UnitsNet.Pressure(point.Y, AdSecGH.DocumentUnits.StressUnit),
                new Oasys.Units.Strain(point.X, AdSecGH.DocumentUnits.StrainUnit)); 
        }
        ...
        {
            IQuantity quantityStrain = new Oasys.Units.Strain(0, AdSecGH.DocumentUnits.StrainUnit);
            string unitStrainAbbreviation = string.Concat(quantityStrain.ToString().Where(char.IsLetter));
            IQuantity quantityStress = new UnitsNet.Pressure(0, AdSecGH.DocumentUnits.StressUnit);
            string unitStressAbbreviation = string.Concat(quantityStress.ToString().Where(char.IsLetter));
            return "AdSec " + TypeName + " {"
                + Math.Round(StressStrainPoint.Strain.As(AdSecGH.DocumentUnits.StrainUnit), 4) + unitStrainAbbreviation + ", "
                + Math.Round(StressStrainPoint.Stress.As(AdSecGH.DocumentUnits.StressUnit), 4) + unitStressAbbreviation + "}";
        }
        ...
        public override bool CastTo<TQ>(out TQ target)
        {
            ...
            if (typeof(TQ).IsAssignableFrom(typeof(IStressStrainPoint)))
            {
                target = (TQ)(object) IStressStrainPoint.Create(
                    new UnitsNet.Pressure(Value.Y, AdSecGH.DocumentUnits.StressUnit),
                    new Oasys.Units.Strain(Value.X, AdSecGH.DocumentUnits.StrainUnit));
                return true;
            }

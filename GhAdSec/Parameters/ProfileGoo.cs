    ...
    public class AdSecProfileGoo : GH_GeometricGoo<Polyline>, IGH_PreviewData
    {
        ...
        private void UpdatePreview()
        {
            ...
            if (m_plane != null)
            {
                if (m_plane != Plane.WorldXY & m_plane != Plane.WorldYZ & m_plane != Plane.WorldZX)
                {
                    ...
                    previewXaxis = new Line(m_plane.Origin, m_plane.XAxis, length.As(AdSecGH.DocumentUnits.LengthUnit));
                    previewYaxis = new Line(m_plane.Origin, m_plane.YAxis, length.As(AdSecGH.DocumentUnits.LengthUnit));
                    previewZaxis = new Line(m_plane.Origin, m_plane.ZAxis, length.As(AdSecGH.DocumentUnits.LengthUnit));
                }
            }
        }
        ...
        internal static List<Point3d> PtsFromAdSecPolygon(IPolygon polygon, Plane local)
        {
            ...
            foreach (IPoint apt in apts)
            {
                Point3d pt = new Point3d(0,
                    apt.Y.As(AdSecGH.DocumentUnits.LengthUnit), 
                    apt.Z.As(AdSecGH.DocumentUnits.LengthUnit)
                    );
        ...
        public override bool CastFrom(object source)
        {
            ...
            if (GH_Convert.ToCurve(source, ref crv, GH_Conversion.Both))
            {
                ...
                {
                    AdSecProfileGoo temp = new AdSecProfileGoo(poly, AdSecGH.DocumentUnits.LengthUnit);

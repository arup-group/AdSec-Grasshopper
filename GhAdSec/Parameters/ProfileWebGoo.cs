...
namespace AdSecGH.Parameters
{
    public class AdSecProfileWebGoo : GH_Goo<IWeb>
    {
        ...
        public override string ToString()
        {
            ...
            {
                UnitsNet.Length thk = this.Value.BottomThickness.ToUnit(DocumentUnits.LengthUnit);
                web += "Constant " + thk.ToString() + "}";
            }
            else
            {
                UnitsNet.Length thk1 = this.Value.TopThickness.ToUnit(DocumentUnits.LengthUnit);
                UnitsNet.Length thk2 = this.Value.BottomThickness.ToUnit(DocumentUnits.LengthUnit);

    ...
    public class AdSecStressStrainCurveGoo : GH_GeometricGoo<Curve>, IGH_PreviewData
    {
        ...
        internal static Oasys.Collections.IList<IStressStrainPoint> StressStrainPtsFromPolyline(PolylineCurve curve)
        {
            ...
            for (int j = 0; j < curve.PointCount; j++)
            {
                ...
                pt = IStressStrainPoint.Create(
                    new UnitsNet.Pressure(point3d.Y, AdSecGH.DocumentUnits.StressUnit),
                    new Oasys.Units.Strain(point3d.X, AdSecGH.DocumentUnits.StrainUnit));
                pts.Add(pt);
            }
            return pts;
        }
        internal static Tuple<Curve, List<Point3d>> Create(IStressStrainCurve stressStrainCurve, StressStrainCurveType type, bool isCompression)
        {
            ...
            if (type == StressStrainCurveType.Bilinear)
            {
                ...
                pts.Add(new Point3d(
                    crv1.YieldPoint.Strain.As(AdSecGH.DocumentUnits.StrainUnit) * direction,
                    crv1.YieldPoint.Stress.As(AdSecGH.DocumentUnits.StressUnit) * direction, 0));
                pts.Add(new Point3d(
                    crv1.FailurePoint.Strain.As(AdSecGH.DocumentUnits.StrainUnit) * direction,
                    crv1.FailurePoint.Stress.As(AdSecGH.DocumentUnits.StressUnit) * direction, 0));
            ...
            else if (type == StressStrainCurveType.Explicit)
            {
                ...
                {
                    pts.Add(new Point3d(
                    pt.Strain.As(AdSecGH.DocumentUnits.StrainUnit) * direction,
                    pt.Stress.As(AdSecGH.DocumentUnits.StressUnit) * direction, 0));
                }
                crvOut = new Polyline(pts).ToPolylineCurve();
            }
            else if (type == StressStrainCurveType.Linear)
            {
                ...
                pts.Add(new Point3d(0, 0, 0));
                pts.Add(new Point3d(
                    crv3.FailurePoint.Strain.As(AdSecGH.DocumentUnits.StrainUnit) * direction,
                    crv3.FailurePoint.Stress.As(AdSecGH.DocumentUnits.StressUnit) * direction, 0));
                crvOut = new Polyline(pts).ToPolylineCurve();
            }
            else
            {
                double maxStrain = stressStrainCurve.FailureStrain.As(AdSecGH.DocumentUnits.StrainUnit);
                List<Point3d> polypts = new List<Point3d>();
                for (int i = 0; i < 100; i++)
                {
                    Oasys.Units.Strain strain = new Oasys.Units.Strain((double)i / (double)100.0 * maxStrain, AdSecGH.DocumentUnits.StrainUnit);
                    
                    UnitsNet.Pressure stress = stressStrainCurve.StressAt(strain);
                        
                    polypts.Add(new Point3d(
                    strain.As(AdSecGH.DocumentUnits.StrainUnit) * direction,
                    stress.As(AdSecGH.DocumentUnits.StressUnit) * direction, 0));
                ...
                if (type == StressStrainCurveType.FibModelCode)
                {
                    ...
                    pts.Add(new Point3d(0, 0, 0));
                    pts.Add(new Point3d(
                        crv.PeakPoint.Strain.As(AdSecGH.DocumentUnits.StrainUnit) * direction,
                        crv.PeakPoint.Stress.As(AdSecGH.DocumentUnits.StressUnit) * direction, 0));
                }
                ...
                if (type == StressStrainCurveType.ParabolaRectangle)
                {
                    ...
                    pts.Add(new Point3d(0, 0, 0));
                    pts.Add(new Point3d(
                        crv.YieldPoint.Strain.As(AdSecGH.DocumentUnits.StrainUnit) * direction,
                        crv.YieldPoint.Stress.As(AdSecGH.DocumentUnits.StressUnit) * direction, 0));
                }

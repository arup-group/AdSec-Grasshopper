...
namespace GhAdSec.UI
{
    ...
    /// </summary>
    public class MultiDropDownComponentUICapsule : GH_ComponentAttributes
    {
        public MultiDropDownComponentUICapsule(GH_Component owner, Action<int, int> clickHandle, List<List<string>> dropdownContents, List<string> selections, List<string> spacerTexts = null, List<string> initialdescriptions = null) : base(owner)
        {
        ...
        protected override void Layout()
        {
            ...
            // override the height if not displaying icons
            Bounds = new RectangleF(Bounds.X, Bounds.Y, Bounds.Width, 28);
        ...
        protected override void Render(GH_Canvas canvas, System.Drawing.Graphics graphics, GH_CanvasChannel channel)
        {
            ...
            //{
            //    RenderIncomingWires(canvas.Painter, comp.Params, Owner.WireDisplay);
            //    return;
            //}
            ...
            if (channel == GH_CanvasChannel.Objects)
            {
                ...
                {
                    RectangleF iconRectangle = ContentBox;
                    iconRectangle.Y += 2;
                    GH_GraphicsUtil.RenderIcon(graphics, iconRectangle, Owner.Locked ? Owner.Icon_24x24_Locked : Owner.Icon_24x24);
                }
                else
                {
                    GH_Palette palette2 = GH_Palette.Black;
                    RectangleF textRectangle = ContentBox;
                    textRectangle.Height = 24;

                    int width = GH_FontServer.StringWidth(Owner.Name, GH_FontServer.Large);
                    textRectangle.Width = width + 4;
                    textRectangle.X = Bounds.X + Bounds.Width / 2 - width / 2 - 2;
                    GH_Capsule contentbx = GH_Capsule.CreateCapsule(textRectangle, palette2);
                    contentbx.Render(graphics, Selected, Owner.Locked, true);
                    ...
                    graphics.DrawString(Owner.Name, GH_FontServer.Large, Brushes.White, textRectangle, format);
                }
